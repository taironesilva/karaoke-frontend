...
function search() {
  ...
  if (
    text_pesquisa.value == 0 ||
    text_pesquisa.value == null ||
    text_pesquisa.length == 0
  ) {
...
async function searchInBd(radio, text) {
  const response = await fetch('http://127.0.0.1:3001/realizarConsulta', {
    method: 'POST',
    headers: {
...
const getFormattedData = (headers, value) =>
  newTableRow(() => headers.map(header => toTableData(header, value)).join(''))
